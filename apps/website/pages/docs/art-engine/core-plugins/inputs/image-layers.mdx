...
This plugin adheres to the base input interface called `InputInterface`, which is a common pattern shared by all input plugins. The interface is defined as follows:
...
```ts copy
ImageLayersInput implements InputInterface<ImageLayersInputInterface>
```
